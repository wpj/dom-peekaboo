...
export type ChangeHandler = (isIntersecting: boolean) => void;
...
export const peekaboo: SetupHandler = (element, onChange, options) => {
  return intersectionObserverSupported()
    ? io(element, onChange, options)
    : scroll(element, onChange, options);
};
